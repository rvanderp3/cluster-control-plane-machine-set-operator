...
func hasVSphereFailureDomains(infrastructure *configv1.Infrastructure) bool {
	if infrastructure == nil {
		return false
	}
	return len(infrastructure.Spec.PlatformSpec.VSphere.FailureDomains) > 0
}
...
func (v VSphereProviderConfig) getFailureDomainFromInfrastructure(fd machinev1.VSphereFailureDomain) (*configv1.VSpherePlatformFailureDomainSpec, error) {
	...
	return nil, errors.New("failure domain is not defined in the infrastructure resource")
}
...
// TO-DO: should we return an error here? a failure domain could fail to be extracted for
// a number of reasons.
...
func newVSphereProviderConfig(logger logr.Logger, raw *runtime.RawExtension, infrastructure *configv1.Infrastructure) (ProviderConfig, error) {
	...
	// if failure domains are specified, workspace, template, and network are derived
	// from the infrastructure resource.
	if hasVSphereFailureDomains(infrastructure) {
