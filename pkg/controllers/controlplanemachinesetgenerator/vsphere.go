...
// buildVSphereFailureDomains builds an VSphere flavored FailureDomains for the ControlPlaneMachineSet.
func buildVSphereFailureDomains(failureDomains *failuredomain.Set) (machinev1.FailureDomains, error) {
	emptyVSphereFailureDomain := machinev1.VSphereFailureDomain{}
	...
	if len(VSphereFailureDomains) > 1 {
		for _, fd := range VSphereFailureDomains {
			if fd == emptyVSphereFailureDomain {
				return emptyFailureDomain, fmt.Errorf("error building VSphere failure domains: %w", errMixedEmptyFailureDomains)
			}
		}
	}

	// We want to make sure that if a failure domain is empty, we don't create it.
	if len(VSphereFailureDomains) == 1 && VSphereFailureDomains[0] == emptyVSphereFailureDomain {
		VSphereFailureDomains = nil
	}
